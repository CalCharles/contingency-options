...
from ChangepointDetection.DynamicsModels import *
...
if __name__ == "__main__":
    ...
    #     model_class = LinearDynamicalPositionFitter
    # elif args.champ_parameters[7] == 1:
    #     model_class = LinearDynamicalVelocityFitter
    # elif args.champ_parameters[7] == 2:
    #     model_class = LinearDynamicalDisplacementFitter

    # params = CHAMP_parameters(args.champ_parameters[0], args.champ_parameters[1], args.champ_parameters[2], args.champ_parameters[3], args.champ_parameters[4], args.champ_parameters[5], args.champ_parameters[6]) # paddle parameters (also change sigma in DynamicsModels)
    # seg_models, changepoints = generate_changepoints([model_class], params, data)
    ...
    # cp_dict[-2] = args.num_frames
    # # print(cp_dict)
    # with open(os.path.join(args.record_rollouts, 'changepoints-' + args.train_edge + '.pkl'), 'wb') as fid:
    #     pickle.dump(cp_dict, fid)
    # Required arguments: record-rollouts (where to store the changepoint, ./runs/FOLDER), train-edge (name of object (Paddle, Ball, etc))
    # num-frames (number of frames for CHAMP counting from the back, champ-parameters 