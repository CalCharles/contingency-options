# Screen
import sys
sys.path.insert(0, "/home/calcharles/research/contingency-a2c/selfBreakout")
import numpy as np
...
class Screen():
	...
	def getState(self):
		self.render_frame()
		return self.extracted_state(), self.frame

	def step(self, action, render=False, duplicate_actions=1, save_path ="", itr=0):
		done = False
		for i in range(duplicate_actions):
			# print("action", action)
			self.actions.take_action(action[0])
			if len(save_path) != 0:
				object_dumps = open(save_path + "/object_dumps.txt", 'a')

				self.write_objects(object_dumps, save_path, itr)
				object_dumps.close()
			...
			for ani_obj in self.animate:
				ani_obj.move()
			extracted_state = {obj.name: obj.getMidpoint() for obj in self.objects}
			if self.ball.losses == 10:
			...
			if render:
				self.render_frame()
		return extracted_state, self.frame, done

	def write_objects(self, object_dumps, save_path, itr):
		state_path = os.path.join(save_path, str(itr//10000))
		try:
		...
		for i, obj in enumerate(self.objects):
			...
			object_dumps.write(obj.name + ":" + " ".join(map(str, obj.getMidpoint())) + " " + str(obj.attribute) + "\t")
		object_dumps.write("\n")
		imio.imsave(os.path.join(state_path, "state" + str(itr % 10000) + ".png"), self.frame)
